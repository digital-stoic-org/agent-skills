...
# Run: bats retrospect/hooks/retrospect-capture.bats
# ==============================================================================
...
# --- detect_project_name: monorepo sub-projects must not collide ----------------
# Source just the function out of the hook (the script's top-level gates exit
# early without a lifecycle event, so we extract the function body directly).
_load_detect() {
  source <(sed -n '/^detect_project_name()/,/^}/p' "$HOOK")
}

@test "detect_project_name: git root -> repo basename" {
  _load_detect
  run detect_project_name "$PROJECT_DIR"
  [ "$status" -eq 0 ]
  [ "$output" = "$(basename "$PROJECT_DIR")" ]
}

@test "detect_project_name: nested sub-project -> distinct dash-joined slug" {
  _load_detect
  mkdir -p "$PROJECT_DIR/projects/alpha" "$PROJECT_DIR/projects/beta"
  local base; base="$(basename "$PROJECT_DIR")"
  run detect_project_name "$PROJECT_DIR/projects/alpha"
  [ "$status" -eq 0 ]
  [ "$output" = "${base}-projects-alpha" ]
  # Distinct sibling must yield a DIFFERENT name (the bug: both collapsed to base).
  run detect_project_name "$PROJECT_DIR/projects/beta"
  [ "$output" = "${base}-projects-beta" ]
  [ "$output" != "$base" ]
}

@test "detect_project_name: empty/unknown cwd -> unknown" {
  _load_detect
  run detect_project_name ""
  [ "$output" = "unknown" ]
  run detect_project_name "unknown"
  [ "$output" = "unknown" ]
}
