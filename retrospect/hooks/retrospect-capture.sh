...
# Returns a filesystem-safe, monorepo-unique project slug for the .retro folder.
# At git root -> basename(git_root). Nested sub-project -> basename + the
# git-relative subpath, dash-joined (e.g. praxis-projects-slasheo-ai-school-tech)
# so distinct sub-projects of one repo don't collide under a single folder.
# Non-git or edge cases -> basename(cwd); empty -> "unknown".
detect_project_name() {
  local cwd="$1"
  ...
  if command -v git &> /dev/null && git -C "$cwd" rev-parse --git-dir &> /dev/null 2>&1; then
    ...
    # `|| true` so a nonzero rev-parse doesn't trip `set -e`.
    git_root=$(git -C "$cwd" rev-parse --show-toplevel 2>/dev/null || true)
    if [ -z "$git_root" ]; then
      # git claimed a repo but toplevel unavailable (bare/worktree edge)
      project_name=$(basename "$cwd")
    else
      base=$(basename "$git_root")
      # Canonicalize cwd so a symlinked path (e.g. an alias dir) still matches the
      # canonical git_root prefix; otherwise the strip below fails and rel bloats.
      local cwd_real
      cwd_real=$(cd "$cwd" 2>/dev/null && pwd -P || printf '%s' "$cwd")
      if [ "$cwd_real" = "$git_root" ]; then rel=""; else rel="${cwd_real#"$git_root"}"; rel="${rel#/}"; fi
      if [ -z "$rel" ] || [ "$rel" = "$cwd_real" ]; then
        # At git root, or prefix-strip failed (cwd not actually under git_root)
        project_name="$base"
      else
        # Nested sub-project: repo basename + dash-joined relative subpath
        project_name="${base}-${rel//\//-}"
      fi
    fi
  else
    # Not a git repo: fall back to current directory name
    project_name=$(basename "$cwd")
  fi

  # Sanitize to a filesystem-safe slug: keep [A-Za-z0-9._-], other runs -> dash,
  # collapse repeated dashes, trim edges (bash-only, no external state).
  project_name=$(printf '%s' "$project_name" | tr -c 'A-Za-z0-9._-' '-')
  while [[ "$project_name" == *--* ]]; do project_name="${project_name//--/-}"; done
  project_name="${project_name#-}"
  project_name="${project_name%-}"

  # If empty after sanitizing, return "unknown"
