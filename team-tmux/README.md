...
Same fleet, same contract, same skill names: one terminal window per agent, one orchestrator, any number of fleet agents that may themselves spawn sub-agents, built for discovery work where the scope moves as the work proceeds, and protecting the human's attention rather than the context window. What differs is the tube. In `team`, `SendMessage` is the default and tmux is one session's declared alternative; here every machine arrow is hard-wired to `send-tmux-message`, which resolves a name against the harness's session registry and pastes into that agent's pane. There is no conditional to read and no second path to take.

Enable it when you want the fleet's traffic **in the open**. Every packet lands as an ordinary turn in a pane you can already see, so a mandate arriving, a report going back and a relay handing over are all events you watch rather than infer; you can type into that same window to redirect the agent mid-flight; and `--archive` puts the packet on disk, which is what makes a relay survivable when the origin leaves right after sending it. The price is that **a name is an address here, not a label** — see below.

> **Enable `team` or `team-tmux`, never both.** They carry the same five skill names on purpose, so the same commands work on either transport and moving a session from one to the other is a matter of switching plugin. Two enabled at once gives you two `/brief` and two `/relay` in the list, which is the one thing that symmetry costs.
...
| `/report` | A stop condition or a cadence checkpoint fires | Fills one REPORT and delivers it to the `orchestrator` named in the mandate, then waits for the redirect. **This skill does not exist in `team`**, which carries the same REPORT template but leaves the sending to `SendMessage`, already in the agent's hands; here the arrow needs a carrier. |
...
It is a fork of `team/references/protocol.md` and is meant to stay a readable one: states, emissions, templates and ownership are word-for-word identical, and the diff between the two files is the **Transport** and **Addressability** sections, the emission table's extra "Carried by" column, and one sentence of the REPORT template's `from` bullet. Keep it that way when you edit either.
