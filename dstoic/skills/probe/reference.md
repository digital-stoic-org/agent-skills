...
1. Write full probe result to `$KNOWLEDGE_DIR/probes/{date}-{slug}-llm.md`
...
Accumulated context cap: 800 tokens total across a chain. At cap, compress prior cycles to 200 tokens (keep: confirmed facts, active constraints, rejected paths). References to `$KNOWLEDGE_DIR` files do not count toward cap.

---

## Knowledge File Convention

Output path: `$KNOWLEDGE_DIR/probes/{date}-{slug}-llm.md`
