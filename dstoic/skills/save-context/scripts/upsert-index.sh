#!/usr/bin/env bash
# Upsert a context row into INDEX.md
# Usage: upsert-index.sh <area> <project> <context> <status_emoji> <focus> <saved_date>
# Example: upsert-index.sh repos agent-skills mystream "🏗️ building" "Working on X" 2026-02-24
#
# Behavior:
# - Finds INDEX.md at git repo root
...
# Check CWD is in code/, projects/, or vaults/
CWD=$(pwd)
if [[ ! "$CWD" =~ /(code|projects|repos|vaults)/ ]]; then
  echo "SKIP: CWD not in code/projects/repos/vaults"
  exit 0
fi
